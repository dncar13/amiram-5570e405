...
-- Ensure unique original_id per type (only if original_id is not null)
CREATE UNIQUE INDEX idx_questions_original_id_type ON questions(original_id, type) WHERE original_id IS NOT NULL;
