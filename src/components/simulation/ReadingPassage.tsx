...
export const ReadingPassage = ({ 
  ...
  if (passageWithLines && passageWithLines.length > 0) {
    return (
      <div className="flex flex-col h-full">
        {title && (
          <div className="text-xl font-bold text-slate-900 px-6 pt-6 pb-3 border-b border-slate-300 bg-slate-50 rounded-t-lg">
            {title}
          </div>
        )}
        <div className="bg-slate-50 rounded-b-lg border border-slate-200 text-slate-800 overflow-y-auto flex-1">
          <div className="space-y-4 leading-relaxed px-6 py-6">
  ...
  return (
    <div className="bg-slate-50 rounded-lg border border-slate-200 text-center flex-1 flex items-center justify-center h-full">
