...
const VocabDictionary: React.FC = () => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === '/' && !e.ctrlKey && !e.metaKey) {
        e.preventDefault();
        document.querySelector<HTMLInputElement>('input[placeholder*="חפש"]')?.focus();
      }
    };
  ...
  const filteredAndSortedWords = useMemo(() => {
    ...
    return filtered;
  }, [searchTerm, levelFilter, categoryFilter, sortBy]);
  ...
  const hasActiveFilters = searchTerm || levelFilter !== 'all' || categoryFilter !== 'all';
  ...
  const WordRow = ({ word }: { word: { id: string; word: string; translation: string; level: string; category: string; example?: string; synonyms?: string[]; collocations?: string[]; wordFamily?: string[] } }) => {
    ...
    const isMarked = markedWords.has(word.id);
          ...
          <div className="max-w-6xl mx-auto px-4 py-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-6">
                <div className="flex items-center gap-2">
                  <Target size={18} />
                  <span className="text-sm">יעד יומי: {todayLearned}/{dailyGoal}</span>
                </div>
                <div className="flex items-center gap-2">
                  <Flame size={18} />
                  <span className="text-sm">{streak} ימים רצופים</span>
                </div>
                <div className="hidden md:block">
                  <Progress value={(todayLearned / dailyGoal) * 100} className="w-32 h-2" />
                </div>
              </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  aria-label={`הצג מילים ברמת קושי קלה (${levelCounts.easy})`}
                >
                  קל ({levelCounts.easy})
                </button>
                <button
                  ...
                  aria-label={`הצג מילים ברמת קושי בינונית (${levelCounts.medium})`}
                >
                ...
                <button
                  ...
                  aria-label={`הצג מילים ברמת קושי קשה (${levelCounts.hard})`}
                >
        ...
        <div className="max-w-6xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <p className="text-sm text-gray-600">
              ...
              {levelFilter !== 'all' && ` · ${levelCounts.easy} קל · ${levelCounts.medium} בינוני · ${levelCounts.hard} קשה`}
            </p>
            {totalCount > 0 && (
              <Link to="/vocab/quiz">
                    ...
                    {filteredAndSortedWords.map(word => (
                      <div key={word.id} className="border rounded-lg p-4 hover:shadow-md transition-shadow">
                        ...
                        <div className="flex justify-end gap-2">
                          <button 
                            onClick={() => toggleMarked(word.id)}
                            className={markedWords.has(word.id) ? 'text-green-600' : 'text-gray-400 hover:text-green-600'}
                            title={markedWords.has(word.id) ? 'מסומנת כמוכרת' : 'סמן כמוכרת'}
                            aria-label={markedWords.has(word.id) ? `המילה ${word.word} מסומנת כמוכרת` : `סמן את המילה ${word.word} כמוכרת`}
                          >
                            <Check size={18} />
                          </button>
                        </div>
