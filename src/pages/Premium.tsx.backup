...
const Premium = () => {
  ...
  const handlePaymentSuccess = async () => {
    ...
    try {
      ...
      // Track successful premium purchase with unique transaction ID
      const immediateTransactionId = `immediate_${Date.now()}_${currentUser?.id}`;
      trackPremiumPurchase({
        ...
        transaction_id: immediateTransactionId
      });
      
      console.log('📊 Immediate purchase tracked:', { transactionId: immediateTransactionId, planType: selectedPlan });
    ...
    if (appliedCoupon?.valid && appliedCoupon.coupon && appliedCoupon.finalAmount === 0) {
      ...
      try {
        ...
        if (result.success) {
          ...
          // Track free order completion with unique transaction ID
          const freeTransactionId = `free_${Date.now()}_${currentUser?.id}_${appliedCoupon.coupon.code}`;
          trackPremiumPurchase({
            plan_type: selectedPlan,
            plan_price: 0,
          ...
          console.log('📊 Free order tracked:', { transactionId: freeTransactionId, planType: selectedPlan, couponCode: appliedCoupon.coupon.code });
