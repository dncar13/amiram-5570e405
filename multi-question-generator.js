...
const { processQuestionsWithTopics, generateListeningTopicsSQL } = require('./topic-mapper.cjs');
  ...
  try {
    ...
    if (types.includes('lc')) {
      ...
      if (aiTopics && lcQuestions.length > 0) {
        ...
        const { results: topicResults } = await processQuestionsWithTopics(lcQuestions, true);
        lcQuestions = topicResults;
      }
    ...
    if (types.includes('cont')) {
      ...
      if (aiTopics && contQuestions.length > 0) {
        ...
        const { results: topicResults } = await processQuestionsWithTopics(contQuestions, true);
        contQuestions = topicResults;
      }
    ...
    if (types.includes('wf')) {
      ...
      if (aiTopics && wfQuestions.length > 0) {
        ...
        const { results: topicResults } = await processQuestionsWithTopics(wfQuestions, true);
        wfQuestions = topicResults;
      }
    ...
    if (types.includes('gc')) {
      ...
      if (aiTopics && gcQuestions.length > 0) {
        ...
        const { results: topicResults } = await processQuestionsWithTopics(gcQuestions, true);
        gcQuestions = topicResults;
      }
